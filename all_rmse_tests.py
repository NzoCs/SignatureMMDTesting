...
class HawkesSimulatorFast:
    ...
    def simulate(self):
        ...
        while t < self.end_time:
            ...
            dt = np.random.exponential(1.0 / lambda_total) if lambda_total > 0 else float("inf")
            t += dt
            if t >= self.end_time:
                break
...
def expected_interarrival_poly(events_history, t_curr, mu, alpha, beta, p):
    ...
    def integral_intensity(u):
        if p == 1.0:
            term = (alpha / beta) * (np.log(history_shifts + beta * u) - np.log(history_shifts))
        else:
            term = (alpha / (beta * (1 - p))) * ( (history_shifts + beta * u)**(1 - p) - history_shifts**(1 - p) )
...
def run_hawkes_opt(ax):
    ...
    for rep in tqdm(range(config.num_rep), desc="Hawkes Opt Repetitions"):
        ...
        for _ in range(config.n_bank):
            ev = simulate_hawkes_thinning_poly(config.mu0, config.alpha0_poly, config.beta0, config.p0, config.T + config.burn_in, config.burn_in)
            events_h0_list.append(ev)

        rmse_h0 = calc_rmse_hawkes_poly(events_h0_list, config.mu0, config.alpha0_poly, config.beta0, config.p0)
        
        for i, alpha1 in enumerate(config.alphas_h1):
            beta1 = config.get_beta1(alpha1)
            mu1 = config.mu1
            rmse_h1 = calc_rmse_hawkes_exp(events_h0_list, mu1, alpha1, beta1)
            results_h0_pred[rep, i] = rmse_h0
            results_h1_pred[rep, i] = rmse_h1
...
def run_hawkes_kernel(ax):
    ...
    for rep in tqdm(range(config.num_rep), desc="Hawkes Kernel Repetitions"):
        events_h0_list = []
        for _ in range(config.n_bank):
            sim = HawkesSimulatorFast(config.mu, config.alpha_exp, config.beta, config.T + config.burn_in, config.burn_in)
            ev = sim.simulate()
            events_h0_list.append(ev)

        rmse_h0 = calc_rmse_hawkes_exp(events_h0_list, config.mu, config.alpha_exp, config.beta)

        for i, p_val in enumerate(config.p_values):
            alpha_p = config.alpha_poly(p_val)
            rmse_h1 = calc_rmse_hawkes_poly(events_h0_list, config.mu, alpha_p, config.beta, p_val)
            results_h0_pred[rep, i] = rmse_h0
            results_h1_pred[rep, i] = rmse_h1
...
def run_hawkes_improved(ax):
    ...
    for rep in tqdm(range(config.num_rep), desc="Hawkes Improved Repetitions"):
        events_h0_list = []
        for _ in range(config.n_bank):
            sim = HawkesSimulatorFast(config.mu0, config.alpha0, config.fixed_beta, config.T + config.burn_in, config.burn_in)
            ev = sim.simulate()
            events_h0_list.append(ev)

        rmse_h0 = calc_rmse_hawkes_exp(events_h0_list, config.mu0, config.alpha0, config.fixed_beta)

        for i, alpha1 in enumerate(config.alphas_h1):
            mu1 = config.get_mu(alpha1)
            rmse_h1 = calc_rmse_hawkes_exp(events_h0_list, mu1, alpha1, config.fixed_beta)
            results_h0_pred[rep, i] = rmse_h0
            results_h1_pred[rep, i] = rmse_h1
