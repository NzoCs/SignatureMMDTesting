...
def sim_hawkes_model(mu, alpha, beta, num_sim, num_time_steps, T, burn_in=100.0):
    ...
    for s in range(num_sim):
        simulator = HawkesSimulator(
            ...
            dim_process=1, start_time=0.0, end_time=T + burn_in,
        )
...
def load_hawkes_paths(mu0, alpha0, mu1, alpha1, beta0, path_bank_size, grid_points, T,
                       beta1=None, normalize=True, burn_in=100.0):
    ...
    if beta1 is None:
        beta1 = beta0
    h0_bank = sim_hawkes_model(mu0, alpha0, beta0, path_bank_size, grid_points, T, burn_in=burn_in)
    h1_bank = sim_hawkes_model(mu1, alpha1, beta1, path_bank_size, grid_points, T, burn_in=burn_in)
...
def plot_errors_vs_delta(type1_results, type2_results, delta_vals,
    ...
    if np.mean(t1_mean) > 10.0:
        print(f"Skipping plot {filename_prefix}: average Type 1 error ({np.mean(t1_mean):.2f}%) > 10%")
        return
...
def main_experiment():
    ...
    # Override DATA_DIR
    DATA_DIR = "eq_hawkes_fixed_mean2"
    ...
    for kernel_name, sig_kernel in KERNELS:
        ...
        def make_fixed_mean_paths(alpha_1, _mu0=mu0, _a0=alpha0, _beta=fixed_beta):
            ...
            h0, h1 = load_hawkes_paths(
                _mu0, _a0, mu_1, alpha_1, _beta,
                path_bank_size=n_paths_delta * 2,  # just enough for the evaluation batch
                grid_points=grid_points, T=T, beta1=_beta, normalize=True
            )
            return h0, h1
        ...
        t1_res, t2_res = compute_errors_vs_delta(
            alphas_h1, make_fixed_mean_paths,
        ...
        print(f"  -> Selected worst case alpha for detailed scenario: alpha={worst_alpha} (Avg T2E: {highest_t2e:.2f}%)")
        ...
        h0_s, h1_s = load_hawkes_paths(
            mu0, alpha0, mu_extreme, alpha_extreme, fixed_beta,
            path_bank_size, grid_points, T, beta1=fixed_beta, normalize=True
        )
