...
from hawkes_fixed_mean_analysis import load_hawkes_paths, plot_type1_error_n, plot_type2_error_n
...
def main():
    ...
    type1_list, type2_list = generate_error_probs_linear_kernel(
        sig_kernel, h0_s, h1_s,
        n_atoms_err, n_paths_err_list, alpha_test, scalings_sweep,
        "ub", num_sim, device,
        filename=f"hawkes_{label}", folder=f"{DATA_DIR}/data/"
    )

    print("Génération des graphiques...")
    plot_type2_error_n(type2_list, scalings_sweep, n_paths_err_list, num_sim,
                       title=f"Alpha=4.0 — Type 2 Error vs Scaling",
                       filename=f"{DATA_DIR}/type2_{label}.svg")

    plot_type1_error_n(type1_list, scalings_sweep, n_paths_err_list, num_sim,
